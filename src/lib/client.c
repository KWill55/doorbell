...
void client_send_image(int sockfd, const Config *config) {
    // make sure all image data is sent correctly
    int num_sent = 0; // how much data actually got sent
    int total_sent = 0;

    while (total_sent < config->payload_size) {
        num_sent = send(sockfd, config->payload + total_sent, config->payload_size - total_sent, 0);
